...
class ServerArgs:
    ...
    disable_trtllm_moe_pdl: A[
        bool,
        Arg(
            help="Launch the flashinfer trtllm-gen MoE kernels without PDL (programmatic dependent launch). Off by default (PDL on); the Inkling model override pins this on because with PDL the chain occasionally never completes when a captured graph replays it concurrently with the shared-expert stream, hanging the whole TP group at its next collective.",
            resolvable=True,
        ),
    ] = False
    pre_warm_nccl: A[
